...
def branin(x):
    ...
    t2 = 10 * (1 - 1 / (8 * np.pi)) * torch.cos(x_0)
...
class GridEnv:
    ...
    def __init__(self, horizon, ndim=2, xrange=[-1, 1], funcs=None,
        ...
        self.funcs = (
            [lambda x: ((torch.cos(x * 50) + 1) * norm.pdf(x * 5)).prod(-1) + 0.01]
            if funcs is None else funcs)
        self.num_cond_dim = len(self.funcs) + 1
        self.xspace = torch.linspace(*xrange, horizon)
    ...
    def obs(self, s=None):
        s = torch.IntTensor(self._state) if s is None else s
        z = torch.zeros(self.num_obs_dim + self.num_cond_dim)
        if self.obs_type == 'one-hot':
            z = torch.zeros((self.horizon * self.ndim + self.num_cond_dim), dtype=torch.float32)
            z[torch.arange(len(s)) * self.horizon + s] = 1
        elif self.obs_type == 'scalar':
            z[:self.ndim] = self.s2x(s)
        elif self.obs_type == 'tab':
            idx = (s * (self.horizon ** torch.arange(self.ndim))).sum()
            z[idx] = 1
    ...
    def s2r(self, s):
        x = self.s2x(s)
        return (self.coefficients * torch.tensor([i(x) for i in self.funcs])).sum() ** self.temperature

    def reset(self, coefs=None, temp=None):
        self._state = torch.IntTensor([0] * self.ndim)
        self._step = 0
        self.coefficients = torch.from_numpy(np.random.dirichlet([1.5] * len(self.funcs))) if coefs is None else coefs
        self.temperature = torch.from_numpy(np.random.gamma(2, 1)) if temp is None else temp
        self.cond_obs = torch.cat([self.coefficients, torch.tensor([self.temperature])])
    ...
    def step(self, a, s=None):
        _s = s
        s = self._state.clone() if s is None else s.clone()
        if a < self.ndim:
            s[a] += 1
    ...
    def state_info(self):
        all_int_states = torch.IntTensor(list(itertools.product(*[list(range(self.horizon))]*self.ndim)))
        state_mask = (all_int_states == self.horizon-1).sum(1) <= 1
        pos = all_int_states[state_mask].float()
        s = pos / (self.horizon-1) * (self.xspace[-1] - self.xspace[0]) + self.xspace[0]
        r = torch.stack([f(s) for f in self.funcs]).T
        return s, r, pos
...
class FlowNet_TBAgent:
    ...
    def sample_many(self, mbsize):
        s = tf(torch.FloatTensor([i.reset()[0] for i in self.envs]))
        done = [False] * mbsize
        ...
        while not all(done):
            ...
            if len(ridx):
                racts = torch.randint(0, cat.logits.shape[1], len(ridx))
                acts[ridx] = racts
            ...
            s = tf(torch.FloatTensor([i[0] for i in step if not i[2]]))
...
def compute_exact_dag_distribution(envs, agent, args):
    env = envs[0]
    stack = [torch.zeros(env.ndim, dtype=torch.int32)]
    state_prob = defaultdict(lambda: torch.zeros(len(envs)))
    ...
    while len(stack):
        ...
        policy = softmax(agent.forward_logits(
            torch.FloatTensor([i.obs(s) for i in envs]))).detach().numpy()
        ...
        for i in range(env.ndim):
            ...
            if tuple(sp) not in opened:
                ...
                stack.append(sp)
    asd.close()
    all_int_states = torch.IntTensor(list(itertools.product(*[list(range(env.horizon))]*env.ndim)))
...
def worker(args, agent, events, outq):
    ...
    agent.envs = [GridEnv(args.horizon, args.ndim, funcs=agent

    .envs[0].funcs) for i in range(mbs)]
